  ...
  return (
    ...
    >
      ...
      <View style={styles.textContainer}>
        ...
        <Text
          style={[styles.titleText, { color: isDarkMode ? "#F9FAFB" : "#111" }]}
          numberOfLines={2}
        >
          {title}
        </Text>
      ...
      <View style={styles.cardBottom}>
        ...
        {routeStatus === "due_soon" && (
          ...
          >
            <View style={[styles.statusButton, { backgroundColor: "#FFCC00" }]}>
              <Text style={[styles.buttonTextWhite, { color: "black" }]}>
                Dipinjam
              </Text>
            </View>
            <Text
        ...
        {routeStatus === "overdue" && (
          <View style={{ alignItems: "flex-end", width: "100%" }}>
            <View
              style={[
                styles.statusButton,
                { backgroundColor: "#FF0000", paddingHorizontal: 15 },
              ]}
            >
              <Text style={styles.buttonTextWhite}>
                RP. {item.fineAmount || "21.000"}
              </Text>
            </View>
          </View>
        )}
        {routeStatus === "in_box" && (
          <View style={{ alignItems: "center", width: "100%" }}>
            ...
            >
              <Text style={[styles.buttonTextWhite, { color: "#007AFF" }]}>
                Waiting
              </Text>
            </View>
          </View>
...
export default function MyLoansScreen() {
  ...
  const activeLoans = loans.filter(
    (l: any) => l.status === "active" && matchesSearch(l),
  );
  const dueSoonLoans = loans.filter(
    (l: any) => l.status === "due_soon" && matchesSearch(l),
  );
  const overdueLoans = loans.filter(
    (l: any) => l.status === "overdue" && matchesSearch(l),
  );
  const waitingLoans = loans.filter(
    (l: any) => ["waiting", "in_box"].includes(l.status) && matchesSearch(l),
  );
  const returnedLoans = loans.filter(
    (l: any) => l.status === "returned" && matchesSearch(l),
  );
      ...
      >
        ...
        <View style={styles.headerRowSpaceBetween}>
          <View
            style={[
              styles.pillFilled,
              { backgroundColor: "#FFCC00", paddingHorizontal: 20 },
            ]}
          >
            <Text style={[styles.pillTextWhite, { color: "black" }]}>
              Due soon
            </Text>
          </View>
        </View>
        ...
        {/* Overdue Section */}
        <View
          style={[styles.headerRowSpaceBetween, { justifyContent: "flex-end" }]}
        >
          <View
            style={[
              styles.pillFilled,
              { backgroundColor: "#FF0000", paddingHorizontal: 20 },
            ]}
          >
        ...
        <View style={styles.headerRowSpaceBetween}>
          <View style={[styles.pillOutline, { borderColor: "#CCC" }]}>
            <Text style={[styles.pillTextDark, { color: "#007AFF" }]}>
              In box
            </Text>
          </View>
        </View>
