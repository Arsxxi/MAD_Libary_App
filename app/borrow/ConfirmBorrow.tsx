...
export default function ConfirmBorrowScreen() {
  ...
  const labelColor = isDarkMode ? "#D1D5DB" : "black";
  ...
  return (
    ...
    >
      ...
      />
      <Text style={[styles.author, { color: isDarkMode ? "#9CA3AF" : "grey" }]}>
        {book?.author}
      </Text>
      <Text style={[styles.title, { color: textColor }]}>{book?.title}</Text>

      <View style={styles.form}>
        <Text style={[styles.label, { color: labelColor }]}>Call number</Text>
        <View style={[styles.inputBox, { borderColor }]}>
          <Text style={{ color: textColor }}>{book?.callNumber}</Text>
        </View>

        <Text style={[styles.label, { color: labelColor }]}>Location</Text>
        <View
          style={[styles.inputBox, { backgroundColor: "#007AFF", borderColor }]}
        >
        ...
        <View style={styles.row}>
          <View style={{ flex: 1, marginRight: 10 }}>
            ...
            {/* ── DROPDOWN ── */}
            <TouchableOpacity
              style={[styles.inputBox, styles.dropdownBox, { borderColor }]}
              onPress={() => setShowDropdown(true)}
            >
              <Text style={{ color: textColor }}>{durationNumber} days</Text>
              <Feather name="chevron-down" size={18} color={textColor} />
            </TouchableOpacity>
          </View>

          <View style={{ flex: 1 }}>
            <Text style={[styles.label, { color: labelColor }]}>Due</Text>
            <View style={[styles.inputBox, { borderColor }]}>
              <Text style={{ color: textColor }}>{formattedDueDate}</Text>
            </View>
          </View>
        </View>
      </View>
      ...
      <Modal visible={showDropdown} transparent animationType="fade">
        ...
        >
          <View style={[styles.dropdownList, { backgroundColor: cardColor }]}>
            <Text
              style={[
